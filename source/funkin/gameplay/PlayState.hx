...
class PlayState extends MusicBeatState
{
	...
	override public function create():Void
	{
		...
		if (SONG.song.toLowerCase() == 'spookeez' || SONG.song.toLowerCase() == 'monster' || SONG.song.toLowerCase() == 'south')
		{
			...
			halloweenBG.frames = FlxAtlasFrames.fromSparrow('assets/images/halloween_bg.png', 'assets/images/halloween_bg.xml');
		...
		switch (curStage)
		{
			...
			case 'mallEvil':
				...
				dad.y -= 80;
			case 'school':
				boyfriend.x += 200; boyfriend.y += 220;
				gf.x += 180;        gf.y += 300;
			case 'schoolEvil':
					...
					new FlxTimer().start(0.1, function(_)
					{
						...
						new FlxTimer().start(0.8, function(_)
						{
							...
							FlxTween.tween(FlxG.camera, {zoom: defaultCamZoom}, 2.5, {
								...
								onComplete: function(_) { startCountdown(); }
							});
						});
					});

				case 'senpai':  schoolIntro(doof);
				case 'roses':   FlxG.sound.play('assets/sounds/ANGRY' + TitleState.soundExt); schoolIntro(doof);
					...
					if (SONG.song.toLowerCase() == 'thorns')
					{
						...
						new FlxTimer().start(0.3, function(swagTimer:FlxTimer)
						{
							...
							else
							{
								...
								new FlxTimer().start(3.2, function(_) { FlxG.camera.fade(FlxColor.WHITE, 1.6, false); });
							}
						});
					}
					else
			...
			switch (swagCounter)
			{
				...
				case 1:
					...
					FlxTween.tween(ready, {y: ready.y += 100, alpha: 0}, Conductor.crochet / 1000,
						{ease: FlxEase.cubeInOut, onComplete: function(_) { ready.destroy(); }});
				...
				case 2:
					...
					FlxTween.tween(set, {y: set.y += 100, alpha: 0}, Conductor.crochet / 1000,
						{ease: FlxEase.cubeInOut, onComplete: function(_) { set.destroy(); }});
				...
				case 3:
					...
					FlxTween.tween(go, {y: go.y += 100, alpha: 0}, Conductor.crochet / 1000,
						{ease: FlxEase.cubeInOut, onComplete: function(_) { go.destroy(); }});
	...
	private function generateSong(dataPath:String):Void
	{
		...
		vocals = (SONG.needsVoices)
			? new FlxSound().loadEmbedded("assets/music/" + curSong + "_Voices" + TitleState.soundExt)
			: new FlxSound();
		...
		for (i in 0...4)
		{
			...
			switch (curStage)
			{
				...
				default:
					...
					switch (i)
					{
						case 0: babyArrow.x += Note.swagWidth * 0; babyArrow.animation.addByPrefix('static', 'arrowLEFT');  babyArrow.animation.addByPrefix('pressed', 'left press',  24, false); babyArrow.animation.addByPrefix('confirm', 'left confirm',  24, false);
						case 1: babyArrow.x += Note.swagWidth * 1; babyArrow.animation.addByPrefix('static', 'arrowDOWN');  babyArrow.animation.addByPrefix('pressed', 'down press',  24, false); babyArrow.animation.addByPrefix('confirm', 'down confirm',  24, false);
						case 2: babyArrow.x += Note.swagWidth * 2; babyArrow.animation.addByPrefix('static', 'arrowUP');    babyArrow.animation.addByPrefix('pressed', 'up press',    24, false); babyArrow.animation.addByPrefix('confirm', 'up confirm',    24, false);
						case 3: babyArrow.x += Note.swagWidth * 3; babyArrow.animation.addByPrefix('static', 'arrowRIGHT'); babyArrow.animation.addByPrefix('pressed', 'right press', 24, false); babyArrow.animation.addByPrefix('confirm', 'right confirm', 24, false);
					}
			}
	...
	private function popUpScore(strumtime:Float):Void
	{
		...
		if (noteDiff > Conductor.safeZoneOffset * 0.9)       { daRating = 'shit'; score = 50; }
		else if (noteDiff > Conductor.safeZoneOffset * 0.75) { daRating = 'bad';  score = 100; }
		else if (noteDiff > Conductor.safeZoneOffset * 0.2)  { daRating = 'good'; score = 200; }
		...
		var seperatedScore:Array<Int> = [
			...
			Math.floor((combo - (Math.floor(combo / 100) * 100)) / 10),
			combo % 10
		];
		...
		FlxTween.tween(comboSpr, {alpha: 0}, 0.2, {
			onComplete: function(_) { coolText.destroy(); comboSpr.destroy(); rating.destroy(); },
	...
	private function keyShit():Void
	{
		...
		playerStrums.forEach(function(spr:FlxSprite)
		{
			switch (spr.ID)
			{
				case 0: if (leftP  && spr.animation.curAnim.name != 'confirm') spr.animation.play('pressed'); if (leftR)  spr.animation.play('static');
				case 1: if (downP  && spr.animation.curAnim.name != 'confirm') spr.animation.play('pressed'); if (downR)  spr.animation.play('static');
				case 2: if (upP    && spr.animation.curAnim.name != 'confirm') spr.animation.play('pressed'); if (upR)    spr.animation.play('static');
				case 3: if (rightP && spr.animation.curAnim.name != 'confirm') spr.animation.play('pressed'); if (rightR) spr.animation.play('static');
			}
	...
	function noteMiss(direction:Int = 1):Void
	{
		...
		boyfriend.stunned = true;
		new FlxTimer().start(5 / 60, function(_) { boyfriend.stunned = false; });
	...
	function fastCarDrive():Void
	{
		...
		new FlxTimer().start(2, function(_) { resetFastCar(); });
	}
